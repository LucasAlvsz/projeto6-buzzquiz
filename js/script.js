...
function saveQuizz(quizzesServer) {
    quizzes = quizzesServer.data
    pickUpUserQuizzes()
}
function erro(erro) {
    console.log(erro);
...
function pickUpUserQuizzes() {
    ...
    for (let i = 0; i < quizzes.length; i++) {
        if (localStorage.getItem(quizzes[i].id.toString())) {
            ...
            userQuizzesList.innerHTML +=
                `<li data-identifier="quizz-card" onclick="showPageQuizz(${quizzes[i].id}) "><p class="quizz user-qzz${i}"><span> <svg onclick="editUserQuizz(${quizzes[i].id})" width="19" height="18" viewBox="0 0 19 18" fill="none"
            ...
            <ion-icon onclick="deleteUserQuizz(${quizzes[i].id}), event.stopPropagation()" name="trash-outline"></ion-icon>
    ...
    if (existsUserQuizzes) {
        ...
        document.querySelector(".user-quizzes span").innerHTML +=
            ...
            <div class="title">
                <p>Seus Quizzes</p>
                <ion-icon data-identifier="create-quizz" onclick="createQuizz()" name="add-circle-sharp"></ion-icon>
            </div>
            `
    }
    ...
    for (let i = 0; i < quizzes.length; i++) {
        if (!userQuizzesIds.includes(quizzes[i].id)) {
            allQuizzesList.innerHTML +=
                `<li data-identifier="quizz-card" onclick="showPageQuizz(quizzes[${i}])"><p class="quizz qzz${i}">${quizzes[i].title}</p>`
